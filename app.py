...
vote_id_elem = response_page.html.xpath('/html/body/article/div[2]/div/div[1]/div/div[2]/div[1]/div[4]/div[1]')
vote_id = (re.sub(";.*","",vote_id_elem[0].attrs['ng-init']).replace("id=","")).strip('\"')
...
json_result_response = json.loads(response_results.text.replace("angular.callbacks._0(","").replace(")",""))
...
for i in range(0,len(ids_elem)):
    ...
    brothers.append(Brother(name= name,id=id,percentage=percentage,num_votes=votes))
